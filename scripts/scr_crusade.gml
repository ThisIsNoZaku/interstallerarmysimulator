    ...
    repeat(305){
        ...
        repeat(capital_number+frigate_number+escort_number){
            ...
            if (obj_ini.name[co,i]!="") and (good=1){
                ...
                if (obj_ini.role[co,i]=obj_img.faction_title[1]) then dead=false;
                ...
                if (dead=true){
                    ...
                    if (obj_ini.role[co,i]=obj_img.faction_title[1]) then command_lost+=1;
...
if (obj_ini.doomed=0){
    if (apoth>0) and (seed>0) then tixt+="  The "+string(apoth)+" surviving "+string(obj_ini.role[100,15])+" were able to recover "+string(seed)+" "+string(obj_img.organics)+".";
    if (apoth=0) and (seed>0) then tixt+="  You had no able-bodied "+string(obj_ini.role[100,15])+", or all of them perished in the Crusade.  Foreign "+string(obj_img.unit[1,15])+" were able to recover "+string(seed)+" of your "+string(obj_img.organics)+".";
}
if (obj_ini.doomed=1){
    tixt+="  Due to fatal mutations in your marines none of the fallen "+string(obj_img.organics)+" was recoverable.";
}

if (artifacts>0) then tixt+="  "+string(artifacts)+" Artifacts were granted to your "+string(obj_img.player_army)+" or looted.";
