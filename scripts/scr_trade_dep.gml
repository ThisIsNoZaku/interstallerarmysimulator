...
repeat(temp2){
    ...
    if (g[i]=obj_img.unit[5,7]) or (g[i]=obj_img.unit[5,17]) then temp1=1;
    ...
    if (temp1=0) and (g[i]="Minor Artifact"){
        ...
        if (rar=1) then scr_add_item(string(obj_img.battlesuit),1);
        if (rar=2) then scr_add_item(obj_img.unit[1,6],1);
        if (rar=3) then scr_add_item(choose("Master Crafted Heavy "+string(obj_img.gyro),"Master Crafted Plasma Pistol"),1);
