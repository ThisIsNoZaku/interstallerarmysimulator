...
if (argument0>0) and (argument1=true){// Adding marines to the drop roster
    ...
    repeat(3500){
        ...
        if (good=1) and (vgood=1) and (i<=100) and (vehy=1) and (obj_ini.veh_lid[comp,i]=argument0){
            ...
            if (obj_ini.veh_role[comp,i]=string(obj_img.unit[1,22])) then speeders+=1;
            if (obj_ini.veh_role[comp,i]=string(obj_img.unit[1,18])) then rhinos+=1;
            if (obj_ini.veh_role[comp,i]=string(obj_img.unit[1,20])) then whirls+=1;
            if (obj_ini.veh_role[comp,i]=string(obj_img.unit[1,19])) then predators+=1;
            if (obj_ini.veh_role[comp,i]=string(obj_img.unit[1,21])) then raiders+=1;
        }
        if (good=1){
            if (obj_ini.race[comp,i]!=0) and (obj_ini.lid[comp,i]=argument0) /*and (string_count("Aspirant",obj_ini.role[comp,i])=0)*/ and (obj_ini.god[comp,i]<10) and ((string_count("Bike",obj_ini.mobi[comp,i])=0) or (vehy=1)) and (para=true){// Man
                ...
                if (obj_ini.role[comp,i]=obj_img.faction_title[1]){master=1;ship_use[argument2]+=1;}
                ...
                if (obj_ini.role[comp,i]="Death Company") and (string_count(obj_img.unit[1,6],obj_ini.armor[comp,i])=0) then mahreens+=1;
                if (obj_ini.role[comp,i]=obj_ini.role[100,4]) then terminators+=1;
                if ((obj_ini.role[comp,i]=obj_ini.role[100,6]) or (obj_ini.role[comp,i]="Venerable "+string(obj_ini.role[100,6])) or (string_count(obj_img.unit[1,6],obj_ini.armor[comp,i])=1)) and (assassinate=false) then dreads+=1;
    ...
    repeat(3500){
        ...
        if (good=1){
            if (obj_ini.race[comp,i]!=0) and (obj_ini.lid[comp,i]=argument0) and (string_count("Aspirant",obj_ini.role[comp,i])=0) and (obj_ini.god[comp,i]<10) and ((string_count("Bike",obj_ini.mobi[comp,i])=0) or (vehy=1)){// Man
                ...
                if (obj_ini.role[comp,i]=obj_img.faction_title[1]){master=0;ship_use[argument2]-=1;}
                if (obj_ini.role[comp,i]=obj_ini.role[100,2]) then honor-=1;
                if (obj_ini.role[comp,i]="Company Champion") then champions-=1;
