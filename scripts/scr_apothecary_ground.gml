    ...
    if (p_player[run]>0){
        ...
        if (p_feature[run]!=""){
            if (string_count("Starship",p_feature[run])>0){
                ...
                if (u2>=tar){// u2=tar;
                    ...
                    new_name=string(obj_img.ship[1,5]);
                    ...
                    obj_ini.ship_class[last_ship]=string(obj_img.ship[1,5]);
                    ...
                    scr_popup("Ancient Ship Restored","The ancient ship within the ruins of "+string(locy)+" has been fully repaired.  It determined to be a "+string(obj_img.ship[1,5])+" vessel and is bristling with golden age weaponry and armor.  Your "+string(obj_ini.role[100,16])+"s are excited; the "+string(obj_img.ship[1,1])+" is ready for its maiden voyage, at your command.","","");                
                }
            }
        }
            ...
            repeat(11){
                if (com+1<=10){
                    ...
                    repeat(300){
                        ...
                        if (obj_ini.wid[com,v]=run) and (obj_ini.loc[com,v]=name) and (obj_ini.armor[com,v]!=obj_img.unit[1,6]) and (apoth>0) and (obj_ini.hp[com,v]<=10) and (normal_hp=true){
            ...
            repeat(11){
                if (com+1<=10){
                    ...
                    repeat(300){
                        v+=1;normal_hp=true;
                        if (obj_ini.race[com,v]>1) then normal_hp=false;
                        if (obj_ini.wid[com,v]=run) and (obj_ini.loc[com,v]=name) and (obj_ini.armor[com,v]!=obj_img.unit[1,6]) and (apoth>0) and (obj_ini.hp[com,v]<=30) and (normal_hp=true){
                            obj_ini.hp[com,v]+=heal;apoth-=1;
                        }
                    }
                }
            ...
            repeat(11){
                if (com+1<=10){
                    ...
                    repeat(300){
                        ...
                        if (obj_ini.role[com,v]!=obj_img.faction_title[1]){
                            if (obj_ini.wid[com,v]=run) and (obj_ini.loc[com,v]=name) and (obj_ini.armor[com,v]!=obj_img.unit[1,6]) and (apoth>0) and (obj_ini.hp[com,v]<100) and (normal_hp=true){
                                obj_ini.hp[com,v]+=heal;apoth-=1;
                        ...
                        if (obj_ini.role[com,v]=obj_img.faction_title[1]){
                            if (string_count("Paragon",string(obj_ini.adv[1])+string(obj_ini.adv[2])+string(obj_ini.adv[3])+string(obj_ini.adv[4]))>0){
                                if (obj_ini.wid[com,v]=run) and (obj_ini.loc[com,v]=name) and (obj_ini.armor[com,v]!=obj_img.unit[1,6]) and (apoth>0) and (obj_ini.hp[com,v]<130) and (normal_hp=true){
                                    obj_ini.hp[com,v]+=heal;apoth-=1;
        ...
        if (apoth>0){
            ...
            repeat(300){
                ...
                if (obj_ini.role[com,v]=obj_img.unit[5,7]){normal_hp=false;mixhp=40;}
                if (obj_ini.role[com,v]=obj_img.unit[5,17]){normal_hp=false;mixhp=40;}
        ...
        if (tick>0){
            repeat(11){
                if (com+1<=10){
                    ...
                    repeat(300){
                        v+=1;
                        if (obj_ini.race[com,v]=1) and (obj_ini.wid[com,v]=run) and (obj_ini.loc[com,v]=name) and (string_count(obj_img.unit[1,6],obj_ini.armor[com,v])>0) and (tick>0) and (obj_ini.hp[com,v]<100){
