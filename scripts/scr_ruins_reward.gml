...
if (loot="gear"){
    ...
    if (argument2<=2) or (argument2>=10){
        wep1=choose("Power Fist",string(obj_img.blade)+"fist","Power Axe","Power Sword");wen1=choose(2,3,4,5);
        wep2=choose("Flamer","Meltagun","Combiflamer","Sniper Rifle");wen2=choose(3,4,5,6,7,8);
        wep3=choose("Missile Launcher","Heavy "+string(obj_img.gyro),string(obj_img.laser)+"cannon","Plasma Pistol");wen3=choose(1,2,3);
    }
    if (argument2=5){
        wep1=choose("Eviscerator");wen1=choose(2,3,4,5);
        wep2=choose("Flamer","Meltagun","Combiflamer");wen2=choose(3,4,5,6,7,8);
        wep3=choose("Heavy Flamer","Heavy "+string(obj_img.gyro),"Plasma Pistol");wen3=choose(1,2,3);
    }
    if (argument2=6){
        wep1=choose(string(obj_img.faction[6])+" Power Sword");wen1=choose(2,3);
...
if (loot="gene_seed"){// Requisition
    ...
    pop.image="geneseed_lab";pop.title="Ancient Ruins: "+string(obj_img.organics);
    pop.text="Your battle brothers have located a hidden, fortified laboratory within the ruins.  Contained are a number of bio-vaults with "+string(obj_img.faction_dude_formal[1])+" "+string(obj_img.organics)+"; "+string(gene)+" in number.  Your marines are not able to determine the integrity or origin.";
    pop.option1="Add the "+string(obj_img.organics)+" to "+string(obj_img.player_army)+" vaults.";
