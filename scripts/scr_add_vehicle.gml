...
if (good!=0){
    ...
    if (obj_ini.fleet_type!=1){// Need a more elaborate ship_carrying += here for the different types of units
        ...
        repeat(30){i+=1;
            if (obj_ini.ship_class[i]=string(obj_img.ship[1,1])) and (first=0) and (obj_ini.ship_capacity[i]>obj_ini.ship_carrying[i]) then first=i;
            if (obj_ini.ship_class[i]=string(obj_img.ship[1,2])) and (backup=0) and (obj_ini.ship_capacity[i]>obj_ini.ship_carrying[i]) then backup=i;
        }
        if (first!=0){
    ...
    if (argument2="standard") and (argument3="standard") and (argument4="standard"){
        if (argument0=string(obj_img.unit[1,18])){obj_ini.veh_wep1[argument1,good]="Storm "+string(obj_img.gyro);obj_ini.veh_wep2[argument1,good]="Storm "+string(obj_img.gyro);obj_ini.veh_upgrade[argument1,good]="Dozer Blades";}
        if (argument0=string(obj_img.unit[1,20])){obj_ini.veh_wep1[argument1,good]="Whirlwind Missiles";obj_ini.veh_wep2[argument1,good]="Storm "+string(obj_img.gyro);obj_ini.veh_upgrade[argument1,good]="";}
        if (argument0=string(obj_img.unit[1,19])){
            var randumb;randumb=choose(1,2,2)obj_ini.veh_upgrade[argument1,good]="Dozer Blades";
            if (randumb=1){obj_ini.veh_wep1[argument1,good]="Autocannon";obj_ini.veh_wep2[argument1,good]="Heavy "+string(obj_img.gyro)+"s";}
            if (randumb=2){obj_ini.veh_wep1[argument1,good]=string(obj_img.laser)+"cannon";obj_ini.veh_wep2[argument1,good]="Lascannons";}
        }
        if (argument0=string(obj_img.unit[1,21])){obj_ini.veh_wep1[argument1,good]="Twin Linked Heavy "+string(obj_img.gyro);obj_ini.veh_wep2[argument1,good]="Twin Linked Lascannon";obj_ini.veh_upgrade[argument1,good]="";}
        if (argument0=string(obj_img.unit[1,22])){obj_ini.veh_wep1[argument1,good]="Heavy "+string(obj_img.gyro);obj_ini.veh_wep2[argument1,good]="";obj_ini.veh_upgrade[argument1,good]="";}
    }
