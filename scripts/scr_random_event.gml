...
Chaos corruption?
The "+string(obj_img.king_title)+"'s divine guidance?
*/
...
if (thatone="sororitas") and (run=true){debugl("RE: Sororitas Company");
    ...
    if (instance_exists(obj_temp5)){
        ...
        if (him.p_type[plan]!="Dead"){
            ...
            if (own!=1) and (him.p_player[plan]<=0) and (him.present_fleet[1]=0) then scr_alert("green","sororitas",string(obj_img.faction[5])+" places a company of "+string(obj_img.faction_dudes[5])+" on "+string(him.name)+" "+string(plan)+".",him.x,him.y);
            if (own=1) or (him.p_player[plan]>0) or (him.present_fleet[1]>0){
                scr_popup("Sororitas","The "+string(obj_img.faction[5])+" have placed a company of "+string(obj_img.faction_dudes[5])+" on "+string(him.name)+" "+string(plan)+".","sororitas","");
...
if (thatone="inquisition_mission") and (run=true) and (known[4]!=0) and (obj_controller.disposition[4]>=0) and (obj_controller.faction_status[4]!="War"){
    ...
    var mishons, d, mishon, you, you2, temp1, temp2, temp3;debugl("RE: "+string(obj_img.faction[4])+" Mission");
    ...
    if (mishons[mishon]="purge"){debugl("RE: Purge");
        ...
        var tixt, tixt2, flit, eta;tixt="The "+string(obj_img.faction[4])+" is trusting you with a special mission.";
        ...
        if (temp1!=3) then scr_popup(string(obj_img.faction[4])+" Mission",tixt,"inquisition","purge|"+string(you2.name)+"|"+string(god)+"|"+string(real(eta+1))+"|");evented=1;
        if (temp1=3) then scr_popup(string(obj_img.faction[4])+" Mission",tixt,"inquisition","cleanse|"+string(you2.name)+"|"+string(god)+"|"+string(real(eta+1))+"|");evented=1;
    }
    ...
    if (mishons[mishon]="spyrer"){debugl("RE: Spyrer");
        ...
        if (instance_exists(obj_temp5)){
            ...
            with(obj_temp5){instance_destroy();}
            tixt="The "+string(obj_img.faction[4])+" is trusting you with a special mission.  An experienced Spyrer on hive world ";
            ...
            scr_popup(string(obj_img.faction[4])+" Mission",tixt,"inquisition","spyrer|"+string(you2.name)+"|"+string(plan)+"|"+string((distan+1))+"|");evented=1;
        }
    }
    
    if (mishons[mishon]="artifact"){var tixt;debugl("RE: Artifact Hold");
        tixt="The "+string(obj_img.faction[4])+" is trusting you with a special mission.  A local "+string(obj_img.faction_dude_formal[4])+" has a powerful artifact.  You are to keep it safe, and NOT use it, until the artifact may be safely retrieved.  Can your chapter handle this mission?";
        scr_popup(string(obj_img.faction[4])+" Mission",tixt,"inquisition","artifact|bop|0|"+string((floor(random(20))+4))+"|");evented=1;
    }
    ...
    if (mishons[mishon]="tomb_world"){debugl("RE: Tomb Bombing");
        ...
        if (instance_exists(obj_temp5)){
            ...
            with(obj_temp5){instance_destroy();}
            tixt="The "+string(obj_img.faction[4])+" is trusting you with a special mission.  They have reason to suspect the "+string(obj_img.faction_dude_formal[13])+" Tomb on planet ";
            if (plan=1) then tixt+=string(you2.name)+" I";if (plan=2) then tixt+=string(you2.name)+" II";
            if (plan=3) then tixt+=string(you2.name)+" III";if (plan=4) then tixt+=string(you2.name)+" IV";
            ...
            scr_popup(string(obj_img.faction[4])+" Mission",tixt,"inquisition","necron|"+string(you2.name)+"|"+string(plan)+"|"+string((distan+1))+"|");evented=1;
        }
    }
    ...
    if (mishons[mishon]="tyranid_organism"){debugl("RE: Gaunt Capture");
        ...
        if (instance_exists(obj_temp5)){
            ...
            if (plan>0){
                ...
                distan=max(distan,6);
                tixt="An "+string(obj_img.faction_dude_formal[3])+" is trusting you with a special mission.  The planet ";
                if (plan=1) then tixt+=string(you2.name)+" I";if (plan=2) then tixt+=string(you2.name)+" II";
                if (plan=3) then tixt+=string(you2.name)+" III";if (plan=4) then tixt+=string(you2.name)+" IV";
                tixt+=" is ripe with "+string(obj_img.faction_dude_formal[9])+" organisms.  They require that you capture one of the Gaunt species for research purposes.  Can your chapter handle this mission?";
                scr_popup(string(obj_img.faction[4])+" Mission",tixt,"inquisition","tyranid_org|"+string(you2.name)+"|"+string(plan)+"|"+string((999))+"|");evented=1;
            }
            with(obj_temp5){instance_destroy();}
        }
    }
    ...
    if (mishons[mishon]="ethereal"){debugl("RE: Ethereal Capture");
        ...
        if (instance_exists(obj_temp5)){
            var you,you2,flit,plan,eta;plan=0;eta=0;
            tixt="The "+string(obj_img.faction[4])+" is trusting you with a special mission.  They require that you capture a "+string(obj_img.faction_formal_dude[8])+" "+string(obj_img.unit[8,14])+" for research purposes.  You have 20 months to locate and capture one.  Can your chapter handle this mission?";
            scr_popup(string(obj_img.faction[4])+" Mission",tixt,"inquisition","ethereal|bop|0|20|");evented=1;
        }
        with(obj_temp5){instance_destroy();}
    }
...
if (thatone="mechanicus_mission") and (run=true) and (known[3]!=0) and (obj_controller.disposition[3]>=50) and (obj_controller.faction_status[3]!="War"){
    with(obj_temp5){instance_destroy();}
    var mishons, d, mishon, you, you2, temp1, temp2, temp3;debugl("RE: "+string(obj_img.faction[3])+" Mission");
    ...
    if (mishons[mishon]="land_raider") or (mishons[mishon]="bionics") or (mishons[mishon]="mars_spelunk"){// show_message("B");
        ...
        if (mishons[mishon]="land_raider") and (instance_exists(you2)){// show_message("E1");
            tixt="The "+string(obj_img.faction[3])+" are trusting you with a special mission.  They wish for you to bring a Land Raider and six "+string(obj_ini.role[100,16])+" to a Forge World for testing and training, for a duration of 24 months.  You have four years to complete this.  Can your chapter handle this mission?";
            scr_popup(string(obj_img.faction[3])+" Mission",tixt,"mechanicus","mech_raider!0!|"+string(you2.name)+"|");evented=1;
        }
        if (mishons[mishon]="bionics") and (instance_exists(you2)){// show_message("E2");
            tixt="The "+string(obj_img.faction[3])+" are trusting you with a special mission.  They desire a squad of "+string(obj_img.faction_dudes_formal[1])+" with bionics to stay upon a Forge World for testing, for a duration of 24 months.  You have four years to complete this.  Can your chapter handle this mission?";
            scr_popup(string(obj_img.faction[3])+" Mission",tixt,"mechanicus","mech_bionics!0!|"+string(you2.name)+"|");evented=1;
        }
        if (mishons[mishon]="mars_spelunk") and (instance_exists(you2)){// show_message("E3");
            tixt="The local "+string(obj_img.faction[3])+" are preparing to embark on a voyage to Mars, to delve into the catacombs in search of lost technology.  Due to your close relations they have made the offer to take some of your "+string(obj_ini.role[100,16])+"s with them.  Can your chapter handle this mission?";
            scr_popup(string(obj_img.faction[3])+" Mission",tixt,"mechanicus","mech_mars|"+string(you2.name)+"|");evented=1;
        }
    }
    ...
    if (mishons[mishon]="necron_study"){debugl("RE: Necron Tomb Study");
        ...
        if (instance_exists(you2)){
            tixt=string(obj_img.faction[3])+" "+string(obj_img.faction_dudes_formal[3])+" have established a research site on a Necron Tomb World.  They are requesting some of your forces to provide security for the research team until the tests may be completed.  Further information is on a need-to-know basis.  Can your chapter handle this mission?";
            scr_popup(string(obj_img.faction[3])+" Mission",tixt,"mechanicus","mech_tomb|"+string(you2.name)+"|");evented=1;
        }
    }
...
if (thatone="inquisition_planet") and (run=true) and (known[4]!=0) and (obj_controller.faction_status[4]!="War"){debugl("RE: Investigate Planet");
    ...
    if (instance_exists(obj_temp5)){
        ...
        tixt="The "+string(obj_img.faction[4])+" wishes for you to investigate "+string(you2.name)+" "+scr_roman(plan)+".";
        ...
        scr_popup(string(obj_img.faction[4])+" Recon",tixt,"inquisition","recon|"+string(you2.name)+"|"+string(plan)+"|"+string(real(eta+1))+"|");
        evented=1;
    }
    with(obj_temp5){instance_destroy();}with(obj_temp6){instance_destroy();}
    with(obj_p_fleet){if (x<-10000){x+=20000;y+=20000;}}
...
if (thatone="rogue_trader") and (run=true){debugl("RE: Rogue Trader");
    ...
    if (instance_exists(obj_temp5)){
        ...
        if (obj_ini.fleet_type!=1) then tixt+="Present "+string(obj_img.ship[1,1])+"s will have access to cheaper wargear for the duration of their visit.";
    ...
    if (that.action="move"){
        // scr_random_find(1,false,"move","");
            ...
            if (instance_exists(that)){
                delay=choose(1,2,2,3);that.action_eta+=delay;tixt=string(obj_img.faction_dude_formal[6])+" pirates have attacked your fleet destined for ";
...
if (thatone="harlequins") and (run=true){debugl("RE: Harlequins");
    ...
    with(obj_temp5){instance_destroy();}
    tixt=string(obj_img.faction_dude_formal[6])+" Harlequins have been seen on planet "+string(that2.name);
...
if (thatone="random_fun") and (run=true){debugl("RE: Random");
    ...
    if (a=5) then stri="Abnormal "+string(obj_img.travel)+" flux in ";
    if (b=1) then stri+="the "+string(obj_img.hq_ground)+".";
    if (b=2) then stri+="Outpost (Name).";
    if (b=3) then stri+="Fleet (Name).";
    if (b=4) then stri+="the Refectory.";
    if (b=5) then stri+="the "+string(obj_img.hq_forge)+".";
    if (b=6) then stri+="the "+string(obj_img.hq_library)+".";
    if (b=7) then stri+="the "+string(obj_img.hq_medical)+".";
    if (b=8) then stri+="the "+string(obj_img.hq_command)+".";
    if (b=9) then stri+="the "+string(obj_img.hq_beastary)+".";
    if (b=10) then stri+="the Hall of Trophies.";
    if (b=11) then stri+="the "+string(obj_img.player_army)+" Crypt.";
    if (b=12) then stri+="the "+string(obj_img.player_army)+" Garage.";
...
if (thatone="warp_storms") and (run=true){debugl("RE: Warp Storm");
    ...
    if (instance_exists(obj_temp5)){
        him=instance_nearest(obj_temp5.x,obj_temp5.y,obj_star);evented=1;
        him.storm+=time;
        if (own!=1) then scr_alert("green","warp",string(obj_img.travel)+" Storms rage across the "+string(him.name)+" system.",him.x,him.y);
        if (own=1) then scr_alert("red","warp",string(obj_img.travel)+" Storms rage across the "+string(him.name)+" system.",him.x,him.y);
        with(obj_temp5){instance_destroy();}
    }
}
...
if (thatone="enemy_forces") and (run=true){var own,him,enem,enem_txt,plan;own=choose(1,1,2,2,3);enem=choose(9,7,10,8,13);debugl("RE: Enemy Forces");
    if (enem=8) then enem_txt=string(obj_img.faction[8]);
    if (enem=7) then enem_txt=string(obj_img.faction[7]);
    if (enem=9) then enem_txt=string(obj_img.faction[9]);
    if (enem=10) then enem_txt=string(obj_img.faction_dude_formal[10]);
    if (enem=13) then enem_txt=string(obj_img.faction_dude_formal[13]);
    if (string_count("Shitty",obj_ini.strin2)=1) then own=1;
...
if (thatone="crusade") and (run=true) and (obj_controller.faction_status[2]!="War"){debugl("RE: Crusade");
    ...
    if (instance_exists(obj_temp5)){
        ...
        scr_popup("Crusade","Fellow "+string(obj_img.faction_dudes_formal[1])+" legions are preparing to embark on a Crusade to a nearby sector.  Your forces are expected at "+string(plant.name)+"; 24 turns from now your ships there shall begin their journey.","crusade","");
        bob=instance_create(obj_temp5.x+16,obj_temp5.y-24,obj_star_event);bob.image_alpha=1;bob.image_speed=1;
...
if (thatone="enemy") and (run=true){debugl("RE: Enemy");
    ...
    if (who=2){event[ev]="enemy_imperium";tixt+=string(obj_img.faction[2]);fucko+=string(obj_img.kingdom_title_shortest);}
    if (who=3){event[ev]="enemy_mechanicus";tixt+=string(obj_img.faction[3]);fucko+=string(obj_img.faction[3]);}
    if (who=4){event[ev]="enemy_inquisition";tixt+=string(obj_img.faction[4]);fucko+=string(obj_img.faction[4]);}
    if (who=5){event[ev]="enemy_ecclesiarchy";tixt+=string(obj_img.faction[5]);fucko+=string(obj_img.faction[5]);}
...
if (thatone="mutation") and (gene_seed>=5) and (run=true){debugl("RE: "+string(obj_img.organics)+" Mutation");
    var tixt;tixt="The "+string(obj_img.player_army)+"'s "+string_lower(obj_img.organics)+" has mutated!  "+string(obj_ini.role[100,15])+"s are scrambling to control the damage and prevent further contamination.  What is thy will?";
    scr_popup(string(obj_img.organics)+" Mutated!",tixt,"gene_bad","");evented=1;
    scr_event_log("red","The "+string(obj_img.player_army)+" "+string(obj_img.organics)+" has mutated.");
}
    ...
    if (instance_exists(obj_p_fleet)){
        ...
        if (instance_exists(obj_temp5)){
            ...
            if (instance_exists(targ2)){
                ...
                tixt="The ";
                if (typ="capital"){tixt+=string(obj_img.ship[1,1])+" '"+string(targ2.capital[the_one])+"'";shippy=targ2.capital_num[the_one];}
                if (typ="frigate"){tixt+=string(obj_img.ship[1,2])+" '"+string(targ2.frigate[the_one])+"'";shippy=targ2.frigate_num[the_one];}
                ...
                tixt+=" has been lost to the miasma of "+string(obj_img.travel_formal)+"!  "
                ...
                if (typ="capital"){
                    ...
                    // obj_ini.ship_location[lol.capital_num[1]]=obj_img.travel;
                }
                if (typ="frigate"){
                    ...
                    // obj_ini.ship_location[lol.frigate_num[1]]=obj_img.travel;
                }
                if (typ="escort"){
                    ...
                    // obj_ini.ship_location[lol.escort_num[1]]=obj_img.travel;
                }
...
if (thatone="chaos_invasion") and (run=true){
    ...
    repeat(90){
        if (q2=0){q+=1;
            if (obj_ini.role[0,q]=obj_img.faction_title[1]){q2=q;
                if (string_count("0",obj_ini.spe[0,q2])>0) then yep2=true;
            }
        }
    }
    
    if (yep!=false) and (woop>0) then scr_popup("The Maw of the "+string(obj_img.travel)+" Yawns Wide","Chief "+string(obj_ini.role[100,17])+" "+string(obj_ini.name[0,5])+" reports that the barrier between the realm of man and the "+string(obj_img.magic_world)+" feels thin and tested.",obj_img.travel,"");
    if ((yep=false) or (woop=0)) and (yep2=true) then scr_popup("The Maw of "+string(obj_img.travel_formal)+" Yawns Wide","The barrier between the realm of man and the "+string(obj_img.magic_world)+" feels thin and tested to you.  Dark forces are afoot.",obj_img.travel,"");
...
if (thatone="necron_awaken") and (run=true) and (known[4]!=0){debugl("RE: Necron Tomb Awakens");
    ...
    if (instance_number(obj_temp5)>0){
        ...
        you2.p_feature[plan]=string_replace(you2.p_feature[plan],string(obj_img.faction_dude_formal[13])+" Tomb|","Awakened "+string(obj_img.faction_dude_formal[13])+" Tomb|");
        tixt=string(you2.name);
        if (plan=1) then tixt+=" I";if (plan=2) then tixt+=" II";if (plan=3) then tixt+=" III";if (plan=4) then tixt+=" IV";
        var om;om=0;
        scr_event_log("red","The "+string(obj_img.faction_dude_formal[13])+" Tomb on "+string(tixt)+" has surged into activity.");
        scr_popup(string(obj_img.faction_dude_formal[13])+" Awakening","The "+string(obj_img.faction_dude_formal[13])+" Tomb on "+string(tixt)+" has surged into activity.  Rank upon rank of the abominations are pouring out from their tunnels.","necron_tomb","");evented=1;
