...
if (argument0>0){// 1 = Fail, 2 = Accomplish, 3 = Clear
    ...
    if (argument1="300req") and (argument0=1){
        ...
        scr_event_log("red",string(obj_img.faction[6])+" Mission Failed: Several years have passed since offering to assist the "+string(obj_img.faction[6])+" with resources.");
    }
    ...
    if (argument1="artifact_loan") and (argument0=1){// Inq want the artifact back
        ...
        if (that=0){
            scr_popup(string(obj_img.faction[4])+" Artifact","The "+string(obj_img.faction[4])+" has asked for the return of the Artifact left in your care.  Despite your Marine's best efforts they were unable to waylay the "+string(obj_img.faction[4])+", who are now furious.  They demand the Artifact's immediate return.","inquisition","");
            scr_event_log("red",string(obj_img.faction[4])+" Mission: The "+string(obj_img.faction[4])+" Artifact entrusted to your "+string(obj_img.player_army)+" is not retrievable.");
        ...
        if (that>0){
            ...
            scr_popup(string(obj_img.faction[4])+" Mission Completed","The "+string(obj_img.faction[4])+" has asked for the return of the Artifact, and your "+string(obj_img.player_army)+" was able to hand it over without complications.  The mission has been accomplished.","inquisition","");
            scr_event_log("",string(obj_img.faction[4])+" Mission Completed: The entrusted Artifact has been returned to the "+string(obj_img.faction[4])+".");
        }
    }
    ...
    if (argument1="300req") and (argument0=2){
        ...
        obj_controller.known[6]+=1;obj_controller.disposition[6]+=10;
    }
