...
repeat(4){run+=1;force=1;
        ...
        repeat(4){s+=1;
            ...
            if (p_player[run]>0) and (p_problem[run,s]="fallen"){
                ...
                if (chan>=3){
                    ...
                    var tixt;tixt="Your "+string(obj_img.faction_dudes[1])+" have scoured "+string(name);
    ...
    if (p_player[run]>0) and ((p_problem[run,1]="bomb") or (p_problem[run,2]="bomb") or (p_problem[run,3]="bomb") or (p_problem[run,4]="bomb")){
        ...
        if (have_bomb>0){
            var tixt;tixt="Your "+string(obj_img.faction_dudes[1])+" on "+string(name);
            if (run=1) then tixt+=" I";if (run=2) then tixt+=" II";if (run=3) then tixt+=" III";if (run=4) then tixt+=" IV";
            tixt+=" are prepared and ready to enter the "+string(obj_img.faction_dude_formal[13])+" Tombs.  A Plasma Bomb is in tow.";
            scr_popup(string(obj_img.faction_dude_formal[13])+" Tomb Excursion",tixt,"necron_cave","blarg|"+string(name)+"|"+string(run)+"|999|");
        }
    }
    ...
    if (p_feature[run]!=""){
        if (string_count("Recruiting World|",p_feature[run])>0) and (obj_controller.gene_seed=0) and (obj_controller.recruiting>0){
            ...
            scr_alert("red","recruiting","The "+string(obj_img.player_army)+" has run out of "+string_lower(obj_img.organics)+"!",0,0);
        }
        ...
        if (string_count("Recruiting World|",p_feature[run])>0) and (obj_controller.gene_seed>0) and (p_owner[run]<=5) and (obj_controller.faction_status[p_owner[run]]!="War"){
            if (p_population[run]>=50){
                ...
                var recr, did, corr, train, dista, onceh;
                recr=999;did=0;corr=10;train=72;dista=0;onceh=0;
                
                if (obj_controller.recruiting=1) then recr=floor(random(300))+1;
                ...
                if (p_type[run]="Agri") and (recr<=10){did=1;corr=choose(1,2,3,4,5,6);train=76;}
                if (p_type[run]="Death") and (recr<=23){did=1;corr=0;train=66;}
                if (p_type[run]="Desert") and (recr<=15){did=1;corr=choose(1,2,3,4,5,6);train=72;}
                if (p_type[run]="Feral") and (recr<=23){did=1;corr=0;train=66;}
                if (p_type[run]="Feudal") and (recr<=10){did=1;corr=choose(1,2,3);train=70;}
                if (p_type[run]="Shrine") and (recr<=10){did=1;corr=0;train=70;}
                if (p_type[run]="Forge") and (recr<=10){did=1;corr=floor(random(10))+floor(random(10))+2;train=72;}
                if (p_type[run]="Hive") and (recr<=5){did=1;corr=floor(random(10))+floor(random(10))+2;train=60;}
                if (p_type[run]="Ice") and (recr<=5){did=1;corr=choose(1,2,3,4,5,6)+choose(1,2,3,4,5,6);train=72;}
                if (p_type[run]="Lava") and (recr<=2){did=1;corr=floor(random_range(3,18))+1;train=72;}
                if (p_type[run]="Temperate") and (recr<=10){did=1;corr=choose(1,2,3,4,5,6);train=72;}
    ...
    if (p_owner[run]=1){
        if (string_count(string(obj_img.hq_ground),p_feature[run])>0){
...
if (chaos_meeting>0){
    ...
    if (master_present=1) and (otm<=21){
        ...
        scr_popup(string(obj_img.faction[11])+" Meeting","A cloaked, ragged figure approaches your forces and hails you.  He is to bring you to meet with their master and you have few enough forces to be permitted.  What is thy will?","chaos_messenger",effect);
    }
    if (master_present=1) and (otm>21){
        scr_popup(string(obj_img.faction[11])+" Meeting","A cloaked, ragged figure approaches your forces and hails you.  He is to bring you to their master, but before the meeting proceeds, you must bring fewer forces.  Only yourself and up to two squads will be allowed in the presence of "+string(obj_controller.faction_title[10])+" "+string(obj_controller.faction_leader[10])+".","chaos_messenger","meeting_2");
        with(obj_temp_meeting){instance_destroy();}
    }
    if (master_present=0) and (otm>21){
        scr_popup(string(obj_img.faction[11])+" Meeting","A cloaked, ragged figure approaches your forces and hails them.  The meeting was supposed to be with the "+string(obj_controller.faction_title[10])+", and yourself, but you are not planet-side.  Land on the planet with up to two squads and the meeting will proceed.","chaos_messenger","meeting_3");
        with(obj_temp_meeting){instance_destroy();}
    }
