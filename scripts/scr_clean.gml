...
/*
    ...
    "+string(obj_img.player_army)+"-Master-Behind-the-Scenes-late-night-edition, where anything
...
if ((hostile_men=1) or (hostile_men=5)) and (men+dreads>0){// show_message("B");
    ...
    repeat(hostile_shots){
        ...
        repeat(700){// This gets a different mahreen if it is not valid
            // if (marine_armor[you]=obj_img.unit[1,6]) or (marine_hp[you]<0){
            if (marine_hp[you]<0){
                if (you=1) then going_up=1;
                if (going_up=0) and (you>0) you-=1;
                if (going_up=1) then you+=1;
        ...
        // if (marine_armor[you]=obj_img.unit[1,6]) or (marine_hp[you]<0) then stahp=1;
