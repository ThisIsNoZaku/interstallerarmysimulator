...
repeat(11){
    ...
    if (ship_apoth>0) then repeat(300){
        ...
        if (maybe=1){
            ...
            if (maybe=2){
                if (obj_ini.armor[co,d]!=obj_img.unit[1,6]) and (ship_apoth>0) and (obj_ini.hp[co,d]<=10) and (normal_hp=true){
    ...
    if (ship_tech>0) then repeat(300){// Initial techmarines repair shit
        ...
        if (string_count(obj_img.unit[1,6],obj_ini.armor[co,d])>0) and (obj_ini.hp[co,d]>0) and (obj_ini.hp[co,d]<50) and (obj_ini.lid[co,d]>0) then maybe=1;
    ...
    if (ship_apoth>0) then repeat(300){
        ...
        if (maybe=1){
            ...
            if (maybe=2){
                if (obj_ini.armor[co,d]!=obj_img.unit[1,6]) and (ship_apoth>0) and (obj_ini.hp[co,d]<=30) and (normal_hp=true){
                    obj_ini.hp[co,d]+=heal;ship_apoth-=1;
                }
            }
        }
    ...
    if (ship_apoth>0) then repeat(300){
        ...
        if (maybe=1){
            ...
            if (maybe=2){
                ...
                if (obj_ini.role[co,d]=obj_img.faction_title[1]){
                    if (string_count("Paragon",string(obj_ini.adv[1])+string(obj_ini.adv[2])+string(obj_ini.adv[3])+string(obj_ini.adv[4]))>0){
                        if (obj_ini.armor[co,d]!=obj_img.unit[1,6]) and (ship_apoth>0) and (obj_ini.hp[co,d]<130) and (normal_hp=true){
                            obj_ini.hp[co,d]+=heal;ship_apoth-=1;
                    ...
                    if (string_count("Paragon",string(obj_ini.adv[1])+string(obj_ini.adv[2])+string(obj_ini.adv[3])+string(obj_ini.adv[4]))=0){
                        if (obj_ini.armor[co,d]!=obj_img.unit[1,6]) and (ship_apoth>0) and (obj_ini.hp[co,d]<100) and (normal_hp=true){
                            obj_ini.hp[co,d]+=heal;ship_apoth-=1;
    ...
    if (ship_apoth>0) then repeat(300){
        ...
        if (maybe=1){
            ...
            if (maybe=2){
                ...
                if (obj_ini.role[co,d]=obj_img.unit[5,7]){normal_hp=false;mixhp=40;}
                if (obj_ini.role[co,d]=obj_img.unit[5,17]){normal_hp=false;mixhp=40;}
    ...
    if (ship_tech>0) then repeat(300){// Last techmarines repair shit
        d+=1;normal_hp=true;maybe=0;
        if (string_count(obj_img.unit[1,6],obj_ini.armor[co,d])>0) and (obj_ini.hp[co,d]>0) and (obj_ini.hp[co,d]<100) and (obj_ini.lid[co,d]>0) then maybe=1;
        if (maybe=1){var c;
            c=0;repeat(capital_number){c+=1;if (obj_ini.lid[co,d]=capital_num[c]) then maybe=2;}
            c=0;repeat(frigate_number){c+=1;if (obj_ini.lid[co,d]=frigate_num[c]) then maybe=2;}
            c=0;repeat(escort_number){c+=1;if (obj_ini.lid[co,d]=escort_num[c]) then maybe=2;}
