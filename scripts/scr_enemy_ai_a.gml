...
repeat(4){run+=1;stop=0;
            ...
            if (p_pdf[run]=0) and (p_player[run]<=0){
                ...
                if (owner<=5){
                    ...
                    if (badd=2) and (p_tyranids[run]=0) and (p_necrons[run]=0) and (p_sisters[run]=0){
                        scr_popup("System Lost","The "+string(name)+" system has been ovewhelmed by "+string(obj_img.faction_dudes_formal[7])+"!","orks","");
                        scr_event_log("red","System "+string(name)+" has been overwhelmed by "+string(obj_img.faction_dudes_formal[7])+".");
    ...
    if (necrons_score>0) and (necrons_attack!="") and (necrons_attack!="player"){
        ...
        if (necrons_attack="pdf"){
            ...
            if (p_pdf[run]=0) and (p_player[run]<=0) and (p_necrons[run]>0){
                ...
                if (badd=1) and (p_tyranids[run]<5) and (p_orks[run]=0) and (p_traitors[run]=0) then scr_alert("red","owner",string(name)+" "+string(run)+" has been overwhelmed by "+string(obj_img.faction_dudes_formal[13])+"!",x,y);
                if (badd=2) and (p_tyranids[run]<5) and (p_orks[run]=0) and (p_traitors[run]=0){
                    scr_popup("System Lost","The "+string(name)+" system has been ovewhelmed by "+string(obj_img.faction_dudes_formal[13])+"!","necron_army","");
                    scr_event_log("red","System "+string(name)+" has been overwhelmed by "+string(obj_img.faction_dudes_formal[13])+".");
                }
            }
    ...
    if (p_owner[run]=7) and (p_player[run]+p_raided[run]>0) and (p_orks[run]=0) and (p_tyranids[run]<4) and (p_chaos[run]=0) and (p_traitors[run]=0) and (p_necrons[run]=0) and (p_tau[run]=0){
        ...
        if (p_first[run]=1){p_owner[run]=1;scr_alert("green","owner",string(obj_img.faction_dudes_formal[7])+" cleansed from "+string(name)+" "+scr_roman(run)+".",x,y);}
        if (p_first[run]=2){p_owner[run]=2;scr_alert("green","owner",string(obj_img.faction_dudes_formal[7])+" cleansed from "+string(name)+" "+scr_roman(run)+".  Control returned to the governor.",x,y);}
        if (p_first[run]=3){p_owner[run]=3;scr_alert("green","owner",string(obj_img.faction_dudes_formal[7])+" cleansed from "+string(name)+" "+scr_roman(run)+".  Control returned to Mechanicus.",x,y);}
    ...
    if (p_owner[run]=8) and (p_player[run]+p_raided[run]>0) and (p_orks[run]=0) and (p_tyranids[run]<4) and (p_chaos[run]=0) and (p_traitors[run]=0) and (p_necrons[run]=0) and (p_tau[run]=0) and (p_pdf[run]=0){
        ...
        if (p_first[run]=1){p_owner[run]=1;scr_alert("green","owner",string(obj_img.faction_dudes_formal[8])+" cleansed from "+string(name)+" "+scr_roman(run)+".",x,y);}
        if (p_first[run]=2){p_owner[run]=2;scr_alert("green","owner",string(obj_img.faction_dudes_formal[8])+" cleansed from "+string(name)+" "+scr_roman(run)+".  Control given to new governor.",x,y);}
        if (p_first[run]=3){p_owner[run]=3;scr_alert("green","owner",string(obj_img.faction_dudes_formal[8])+" cleansed from "+string(name)+" "+scr_roman(run)+".  Control returned to Mechanicus.",x,y);}
        if (dispo[run]=101) then p_owner[run]=1;
    }
    if (p_owner[run]=10) and (p_player[run]+p_raided[run]>0) and (p_orks[run]=0) and (p_tyranids[run]<4) and (p_chaos[run]=0) and (p_traitors[run]=0) and (p_necrons[run]=0) and (p_tau[run]=0){
        ...
        if (p_first[run]=1){p_owner[run]=1;scr_alert("green","owner",string(obj_img.faction_dudes_formal[11])+" cleansed from "+string(name)+" "+scr_roman(run)+".",x,y);}
        if (p_first[run]=2){p_owner[run]=2;scr_alert("green","owner",string(obj_img.faction_dudes_formal[11])+" cleansed from "+string(name)+" "+scr_roman(run)+".  Control returned to the governor.",x,y);}
        if (p_first[run]=3){p_owner[run]=3;scr_alert("green","owner",string(obj_img.faction_dudes_formal[11])+" cleansed from "+string(name)+" "+scr_roman(run)+".  Control returned to Mechanicus.",x,y);}
        if (dispo[run]=101) then p_owner[run]=1;
    }
    if (p_owner[run]=13) and (p_player[run]+p_raided[run]>0) and (p_orks[run]=0) and (p_tyranids[run]<4) and (p_chaos[run]=0) and (p_traitors[run]=0) and (p_necrons[run]=0) and (p_tau[run]=0){
        if (string_count("Awakened|",p_feature[run])=0){
            ...
            if (p_first[run]=1){p_owner[run]=1;scr_alert("green","owner",string(obj_img.faction_dudes_formal[13])+" cleansed from "+string(name)+" "+scr_roman(run)+".",x,y);}
            if (p_first[run]=2){p_owner[run]=2;scr_alert("green","owner",string(obj_img.faction_dudes_formal[13])+" cleansed from "+string(name)+" "+scr_roman(run)+".  Control returned to the governor.",x,y);}
            if (p_first[run]=3){p_owner[run]=3;scr_alert("green","owner",string(obj_img.faction_dudes_formal[13])+" cleansed from "+string(name)+" "+scr_roman(run)+".  Control returned to Mechanicus.",x,y);}
            if (dispo[run]=101) then p_owner[run]=1;
        }
    }
