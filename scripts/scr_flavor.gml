...
if (argument0=-51) then wepp="Heavy "+string(obj_img.gyro)+" Emplacement";
...
if (string_count("&",wepp)>0) or (string_count("|",wepp)>0){// Artifact description
    if (string_count(string(obj_img.gyro),wepp)>0) then wepp=string(obj_img.gyro);
...
if (string_count(string(obj_img.gyro_adj)+" ",wepp)>0) and (solod=false){flavored=1;
...
if (string_count(string(obj_img.gyro_adj)+" ",wepp)>0) and (solod=true){flavored=1;
...
if ((wepp=string(obj_img.laser)+"cannon") or (wepp="Twin Linked Lascannon") or (wepp="Lascannons")) and (solod=false){flavored=1;
    if (wepp="Lascannons"){wepp=string(obj_img.laser)+"cannon";argument3=argument3*2;}
...
if (wepp=string(obj_img.blade)+"sword") and (solod=false){flavored=1;
    if (argument3=1) then p1="A "+string(duhs)+" is struck by a "+string(wepp)+".";
...
/*if (cm_kill=1) and (argument4>0){
    ...
    if (duhs=obj_img.unit[1,6]) or (duhs="Battle Wagon") then obj_ini.master_ork_vehicles+=argument4;
