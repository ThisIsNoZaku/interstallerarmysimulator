...
if (argument0="mars_spelunk"){
    ...
    repeat(11){com+=1;i=0;
        repeat(300){i+=1;
            if (obj_ini.role[com,i]=obj_ini.role[100,16]) and (obj_ini.loc[com,i]=string(obj_img.faction_short[3])+" Vessel"){
                ...
                if (roll2>50){
                    argument1.p_player[argument2]+=scr_unit_size(obj_ini.armor[com,i],obj_ini.role[com,i],true);
    ...
    var tixt;tixt="The journey into the Mars Catacombs was a success.  Your "+string(techs_alive)+" remaining "+string(obj_ini.role[100,16])+"s were useful to the "+string(obj_img.faction[3])+" forces and return with a bounty.  They await retrieval at "+string(argument1.name)+" "+scr_roman(argument2)+".#";
    ...
    scr_popup(string(obj_img.faction[3])+" Mission Completed",tixt,"mechanicus","");
    tixt=string(obj_img.faction[3])+" Mission Completed: "+string(techs_alive)+"/"+string(techs_alive+techs_lost)+" of your "+string(obj_ini.role[100,16])+"s return with ";
...
if (argument0="mech_raider"){
    ...
    if (result="New"){
        scr_popup(string(obj_img.faction[3])+" Mission Completed","Your "+string(obj_ini.role[100,16])+" have worked with the "+string(obj_img.faction[3])+" in a satisfactory manor.  The testing and training went well, but your "+string(obj_img.unit[1,21])+" was ultimately lost.  300 Requisition has been given to your "+string(obj_img.player_army)+" and relations are better than before.","mechanicus","");
        obj_controller.requisition+=300;obj_controller.disposition[3]+=2;
        var com,i,onceh;onceh=0;com=-1;i=0;
        repeat(11){
            if (onceh=0){com+=1;i=0;
                repeat(100){i+=1;
                    if (obj_ini.veh_role[com,i]=string(obj_img.unit[1,21])) and (obj_ini.veh_loc[com,i]=argument1.name) and (obj_ini.veh_wid[com,i]=argument2){
                        onceh=1;
                        obj_ini.veh_race[com,i]=0;obj_ini.veh_loc[com,i]="";obj_ini.veh_name[com,i]="";obj_ini.veh_role[com,i]="";
                        obj_ini.veh_lid[com,i]=0;obj_ini.veh_wid[com,i]=0;obj_ini.veh_wep1[com,i]="";obj_ini.veh_wep2[com,i]="";
                        obj_ini.veh_upgrade[com,i]="";obj_ini.veh_hp[com,i]=0;obj_ini.veh_chaos[com,i]=0;
    ...
    if (result="Land Raider"){
        scr_popup(string(obj_img.faction[3])+" Mission Completed","Your "+string(obj_ini.role[100,16])+" have worked with the "+string(obj_img.faction[3])+" in a satisfactory manor.  The testing and training went well, but your "+string(obj_img.unit[1,21])+" was ultimately lost.  A new "+string(obj_img.unit[1,21])+" has been provided in return.","mechanicus","");
    ...
    if (result="Requisition"){
        scr_popup(string(obj_img.faction[3])+" Mission Completed","Your "+string(obj_ini.role[100,16])+" have worked with the "+string(obj_img.faction[3])+" in a satisfactory manor.  The testing and training went well, but your "+string(obj_img.unit[1,21])+" was ultimately lost.  600 Requisition has been given to your "+string(obj_img.player_army)+" as compensation.","mechanicus","");
...
if (argument0="mech_bionics"){
    ...
    if (result="Marines Lost"){
        scr_popup(string(obj_img.faction[3])+" Mission Completed","The "+string(obj_img.faction[3])+" have finished experimenting on your marines- unfortunantly none of them have survived.  150 Requisition has provided as weregild for each Astartes lost.","mechanicus","");
            ...
            if (onceh<10){com+=1;i=0;
                repeat(300){i+=1;
                    if (obj_ini.race[com,i]=1) and (obj_ini.loc[com,i]=argument1.name) and (obj_ini.wid[com,i]=argument2){onceh+=1;
                        ...
                        if (obj_ini.role[com,i]=obj_img.faction_title[1]){obj_controller.alarm[7]=10;global.defeat=3;}
                        ...
                        if (obj_ini.armor[com,i]!="") then scr_add_item(obj_ini.armor[com,i],1);
    ...
    if (result="Bionics"){
        scr_popup(string(obj_img.faction[3])+" Mission Completed","The "+string(obj_img.faction[3])+" have finished experimenting on your marines.  All of your astartes have survived, though they refuse to speak of the experience.  A large amount of additional Bionics have been provided by the Mechanicus as a reward.","mechanicus","");
    ...
    if (result="Requisition"){
        scr_popup(string(obj_img.faction[3])+" Mission Completed","The "+string(obj_img.faction[3])+" have finished experimenting on your marines.  All of your "+string(obj_img.faction_dudes_formal[1])+" have survived, though they refuse to speak of the experience.  200 Requisition has been provided by the "+string(obj_img.faction[3])+" as a reward.","mechanicus","");
