...
repeat(4){i+=1;
    ...
    if (p_halp[i]=0) and (p_population[i]>0) and (p_owner[i]<6) and (p_owner[i]!=1) and (present_fleet[1]<=0) and (p_player[i]<=0){
        ...
        if (p_orks[i]>=minimum){enemy1=string(obj_img.faction_dude_formal[7]);enemies+=1;}
        if (p_tau[i]>=minimum){enemy1=string(obj_img.faction_dude_formal[8]);enemies+=1;}
        if (p_traitors[i]>=minimum){enemy1=string(obj_img.faction_dude_formal[10]);enemies+=1;}
        if (p_chaos[i]>=minimum){enemy1=string(obj_img.faction_dude_formal[11]);enemies+=1;}
        if (p_necrons[i]>=minimum){enemy1=string(obj_img.faction_dude_formal[13]);enemies+=1;}
        if (p_tyranids[i]>=minimum) and (vision>0) and (p_tyranids[i]>3){enemy1=string(obj_img.faction_dude_formal[9]);enemies+=1;}
    ...
    repeat(4){wob+=1;
        if (p_problem[i,wob]="succession") and (p_timer[i,wob]=0){
            ...
            if (p_owner[i]=2) and (result="chaos"){
                tixt+=" with "+string(obj_img.faction_dude_formal[11])+" in control.";dispo[wob]=0;
                p_owner[i]=10;p_pdf[i]+=p_guardsmen[i];p_guardsmen[i]=0;scr_alert("red","succession",string(tixt),x,y);
            }
            if (p_owner[i]=2) and (result="tau"){
                tixt+=" with a "+string(obj_img.faction_dude_formal[8])+" sympathizer in control.";dispo[wob]=10+choose(1,2,3,4,5,6);
            ...
            if (string_pos("mech_",p_problem[i,wob])>0){
                if (p_timer[i,wob]>0){
                    if (string_count("mech_raider",p_problem[i,wob])>0){
                        ...
                        check2=scr_vehicle_count(string(obj_img.unit[1,21]),string(name)+"|"+string(i)+"|");
                        if (check1>=6) and (check2>=1){t2+=1;scr_alert("","mission",string(obj_img.faction[3])+" Mission on "+string(name)+" "+scr_roman(i)+" is "+string(round((t2/24)*100))+"% complete.",0,0);}
                    ...
                    if (string_count("mech_bioni",p_problem[i,wob])>0){
                        ...
                        if (check1>=10){t2+=1;scr_alert("","mission",string(obj_img.faction[3])+" Mission on "+string(name)+" "+scr_roman(i)+" is "+string(round((t2/24)*100))+"% complete.",0,0);}
                    ...
                    if (string_count("mech_tomb2",p_problem[i,wob])>0){
                        ...
                        if (t2>0){
                            ...
                            if (battli>0) and (p_player[i]<=0){// XDDDDD
                                scr_popup(string(obj_img.faction[3]),"The "+string(obj_img.faction[3])+" Research team on planet "+string(name)+" "+scr_roman(i)+" have been killed by "+string(obj_img.faction_dudes_formal[13])+" in the absence of your astartes.  The "+string(obj_img.faction[3])+" are absolutely livid, doubly so because of the promised security they did not recieve.","","");
                        ...
                        if (t2>3) and (battli=0){// Done
                            ...
                            if (roll1>40) then scr_alert("","mission",string(obj_img.faction[3])+" research within the Necron Tomb of "+string(name)+" "+scr_roman(i)+" continues.",0,0);
                            ...
                            if (roll1<=40){// Complete
                                ...
                                if (reward=1){obj_controller.requisition+=400;
                                    text="The "+string(obj_img.faction[3])+" Research team on planet "+string(name)+" "+scr_roman(i)+" have completed their work without any major setbacks.  Pleased with your "+string(obj_img.faction_dudes_formal[1])+"' work, they have granted you 400 Requisition to be used as you see fit.";
                                    scr_event_log("",string(obj_img.faction[3])+" Mission Completed: The "+string(obj_img.faction[3])+" research team on "+string(name)+" "+scr_roman(i)+" have completed their work.");
                                }
                                if (reward=2){
                                    ...
                                    text="The "+string(obj_img.faction[3])+" Research team on planet "+string(name)+" "+scr_roman(i)+" have completed their work without any major setbacks.  Pleased with your "+string(obj_img.faction_dudes_formal[1])+"' work, they have granted your "+string(obj_img.player_army)+" an artifact, to be used as you see fit.";
                                    scr_event_log("",string(obj_img.faction[3])+" Mission Completed: The "+string(obj_img.faction[3])+" research team on "+string(name)+" "+scr_roman(i)+" have completed their work.");
                                    scr_event_log("","Artifact gifted from "+string(obj_img.faction[3])+".");
                                }
                                
                                scr_popup(string(obj_img.faction[3])+" Mission Completed",text,"mechanicus","");
                    ...
                    if (p_problem[i,wob]="mech_tomb1") and (p_player[i]>=20){
                        ...
                        if (check>=20){
                            ...
                            scr_popup(string(obj_img.faction[3])+" Research","The "+string(obj_img.faction[3])+" Research team on planet "+string(name)+" "+scr_roman(i)+" has taken note of your "+string(obj_img.faction_dudes_formal[1])+" and are now prepared to begin their research.  Your marines are to stay on the planet until further notice.","necron_cave","");
                        }
                    }// End necron_tomb1
                }
                if (p_timer[i,wob]=0){
                    ...
                    if (string_count("mech_mars",p_problem[i,wob])>0){
                            ...
                            repeat(300){ide+=1;
                                if (obj_ini.role[com,ide]=obj_ini.role[100,16]){
                                    ...
                                    if (obj_ini.loc[com,ide]=name) and (obj_ini.wid[com,ide]=i){
                                        p_player[i]-=scr_unit_size(obj_ini.armor[com,ide],obj_ini.role[com,ide],true);
                                        obj_ini.loc[com,ide]=string(obj_img.faction_short[3])+" Vessel";obj_ini.wid[com,ide]=0;obj_ini.lid[com,ide]=0;
                                        techs_taken+=1;
                                    }
                                    if (obj_ini.lid[com,ide]>0){
                                        ship_planet=obj_ini.ship_location[obj_ini.lid[com,ide]];
                                        if (ship_planet=name){
                                            obj_ini.ship_carrying[obj_ini.lid[com,ide]]-=scr_unit_size(obj_ini.armor[com,ide],obj_ini.role[com,ide],true);
                                            obj_ini.loc[com,ide]=string(obj_img.faction_short[3])+" Vessel";obj_ini.wid[com,ide]=0;obj_ini.lid[com,ide]=0;
                                            techs_taken+=1;
                                        }
                                    }
                        ...
                        if (techs_taken=0){
                            var tixt;tixt=string(obj_img.faction[3])+" Mission Failed: Journey to Mars Catacombs at "+string(name)+" "+scr_roman(i)+".";
                        ...
                        if (techs_taken>0){
                            ...
                            var taxt;taxt=string(obj_img.faction[3])+" Ship departs for the Mars catacombs.  Onboard are "+string(techs_taken)+" of your "+string(obj_ini.role[100,16])+"s.";
                    ...
                    if (p_problem[i,wob]="mech_tomb1"){
                        var tixt;tixt=string(obj_img.faction[3])+" Mission Failed: Necron Tomb Study at "+string(name)+" "+scr_roman(i)+".";
                        scr_alert("red","mission_failed",string(tixt),0,0);scr_event_log("red",tixt);
                        obj_controller.disposition[3]-=15;
                    }
                    if (string_count("mech_raider",p_problem[i,wob])>0){
                        var tixt;tixt=string(obj_img.faction[3])+" Mission Failed: Land Raider testing at "+string(name)+" "+scr_roman(i)+".";
                        scr_alert("red","mission_failed",string(tixt),0,0);scr_event_log("red",tixt);
                        obj_controller.disposition[3]-=6;
                    }
                    if (string_count("mech_bioni",p_problem[i,wob])>0){
                        var tixt;tixt=string(obj_img.faction[3])+" Mission Failed: Experiment on Astartes bionics at "+string(name)+" "+scr_roman(i)+".";
                        scr_alert("red","mission_failed",string(tixt),0,0);scr_event_log("red",tixt);
                        obj_controller.disposition[3]-=6;
                    }
                    p_problem[i,wob]="";
        ...
        if (p_problem[i,wob]="recon") and (p_timer[i,wob]=0){
            var tixt;tixt=string(obj_img.faction[4])+" Mission Failed: Investigate ";
        ...
        if (p_problem[i,wob]="bomb") and (p_timer[i,wob]=0){
            var tixt;
            tixt="The "+string(obj_img.faction_dude_formal[13])+" Tomb of planet ";
            ...
            tixt+=" has not been deactivated in time.  It has awakened, rank upon rank of "+string(obj_img.faction_dudes_formal[13])+" pouring out to the planet's surface.  The "+string(obj_img.faction[4])+" is not pleased with your failure.";
            scr_popup(string(obj_img.faction[4])+" Mission Failed",tixt,"necron_army","");
            if (i=1) then scr_event_log("red",string(obj_img.faction[4])+" Mission Failed: Bombing run failed; the "+string(obj_img.faction_dude_formal[13])+" Tomb on "+string(name)+" I has become active.");
            if (i=2) then scr_event_log("red",string(obj_img.faction[4])+" Mission Failed: Bombing run failed; the "+string(obj_img.faction_dude_formal[13])+" Tomb on "+string(name)+" II has become active.");
            if (i=3) then scr_event_log("red",string(obj_img.faction[4])+" Mission Failed: Bombing run failed; the "+string(obj_img.faction_dude_formal[13])+" Tomb on "+string(name)+" III has become active.");
            if (i=4) then scr_event_log("red",string(obj_img.faction[4])+" Mission Failed: Bombing run failed; the "+string(obj_img.faction_dude_formal[13])+" Tomb on "+string(name)+" IV has become active.");
            
            p_problem[i,wob]="";p_timer[i,wob]=0;p_necrons[i]=4;
            if (string_count("Awakened",p_feature[i])=0) then p_feature[i]=string_replace(p_feature[i],string(obj_img.faction_dude_formal[13])+" Tomb|","Awakened "+string(obj_img.faction_dude_formal[13])+" Tomb|");
        ...
        if (p_problem[i,wob]="spyrer") and (p_timer[i,wob]=0){
            var tixt,text;
            tixt="The Spyrer on "+string(name)+" "+scr_roman(i)+" has been left unchecked.  In the ensuing carnage some high-ranking officials have been killed, along with several Nobles.  Panic is running amock in several parts of the hives and the "+string(obj_img.faction[4])+" is less than pleased.";
            text=string(obj_img.faction[4])+" Mission Failed: The Spyrer on "+string(name)+" "+scr_roman(i)+" was not removed.";
            scr_popup(string(obj_img.faction[4])+" Mission Failed",tixt,"spyrer","");
...
if (storm>0){storm-=1;
    if (storm=0){
        var tr;tr=string(obj_img.travel)+" Storms over "+string(name)+" dissipate.";
