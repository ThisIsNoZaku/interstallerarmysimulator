...
repeat(4){
    ...
    if (p_owner[i]!=10) and (p_owner[i]!=6) and (planets>=i) and (p_type[i]!="Dead") and (p_type[i]!="Craftworld"){contin=0;
        ...
        if (contin>0) and (p_pdf[i]=0) and (p_guardsmen[i]=0) and (p_tau[i]=0) and (p_orks[i]=0){p_owner[i]=10;
            scr_alert("red","owner",string(name)+" "+string(i)+" has fallen to "+string_lower(obj_img.faction_dudes_formal[10])+"!",x,y);
        }
        ...
        if (contin>0) and (p_type[i]!="Space Hulk"){
            ...
            if (rando<=40){
                ...
                if (p_traitors[i]=0){
                    ...
                    if (p_pdf[i]=0) then tixt=string(obj_img.faction_dude_formal[10])+" cults have appeared in "+string(name)+" "+string(i)+".";
            ...
            if (rando>=41) and (rando<81) and (p_traitors[i]<1){p_traitors[i]=2;tixt=string(obj_img.faction_dude_formal[10])+" cults have appeared in "+string(name)+" "+string(i)+".";}// Minor uprising
            if (rando>=81) and (rando<91) and (p_traitors[i]<3){p_traitors[i]=3;tixt=string(obj_img.faction_dude_formal[10])+" cults have spread around "+string(name)+" "+string(i)+".";}// Major uprising
            if (rando>=91) and (rando<100) and (p_traitors[i]<4){notixt=true;
                ...
                scr_popup("Heretic Revolt","A massive "+string_lower(obj_img.faction_dude_formal[10])+" uprising on "+string(name)+" "+scr_roman(i)+" threatens to plunge the star system into chaos.","chaos_cultist","");
                scr_alert("red","owner","Massive "+string_lower(obj_img.faction_dude_formal[10])+" uprising on "+string(name)+" "+scr_roman(i)+".",x,y);
                scr_event_log("purple","Massive "+string_lower(obj_img.faction_dude_formal[10])+" uprising on "+string(name)+" "+scr_roman(i)+".");
            }// Huge uprising
    ...
    repeat(4){i+=1;
        ...
        if (i<=planets) and (p_influence[i]>=70) and (p_owner[i]!=8) and (p_owner[i]!=10) and (p_owner[i]!=7) and (p_owner[i]!=9) and (p_type[i]!="Space Hulk"){
            ...
            if (ran3>=95) and (p_orks[i]=0) and (p_traitors[i]=0) and (p_necrons[i]=0) and (p_demons[i]=0) and (p_chaos[i]=0){
                ...
                if (badd=1) then scr_alert("red","owner","Planet "+string(name)+" "+string(i)+" has succeeded to the "+string(obj_img.faction[8])+"!",x,y);
                if (badd=2){
                    scr_popup("System Lost","The "+string(name)+" system has been taken by the "+string(obj_img.faction[8])+"!","tau","");owner=8;
                    scr_event_log("red","System "+string(name)+" has been taken by the "+string(obj_img.faction[8])+".");
                }
