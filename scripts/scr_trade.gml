...
repeat(4){i+=1;
    ...
    if (trade_give[i]=string(obj_img.organics)) and (trade_mnum[i]>0){
    ...
    if (trade_take[i]=string(obj_img.battlesuit)) then their_worth+=trade_tnum[i]*400;
    if (trade_take[i]="Tartaros") then their_worth+=trade_tnum[i]*900;
    if (trade_take[i]=string(obj_img.unit[1,21])) then their_worth+=trade_tnum[i]*600;
    ...
    if (trade_take[i]=obj_img.unit[5,7]) then their_worth+=trade_tnum[i]*50;
    if (trade_take[i]=obj_img.unit[5,17]) then their_worth+=trade_tnum[i]*70;
    
    if (trade_take[i]=string(obj_img.faction[6])+" Power Sword") then their_worth+=trade_tnum[i]*50;
...
if (rando4<=deal) and (trading_artifact=0){
    ...
    /*repeat(4){i+=1;
        ...
        if (trade_give[i]=string(obj_img.organics)) and (trade_mnum[i]>0) then gene_seed-=trade_mnum[i];
